...
/// struct for typed errors of method [`get_services_product_log_explorer_insights`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetServicesProductLogExplorerInsightsError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_services_product_log_explorer_insights(configuration: &mut configuration::Configuration) -> Result<crate::models::LogExplorerInsightsResponseBodyGetAllServices, Error<GetServicesProductLogExplorerInsightsError>> {
    let local_var_configuration = configuration;
    ...
    let local_var_uri_str = format!("{}/enabled-products/v1/log_explorer_insights/services", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetServicesProductLogExplorerInsightsError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
