...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetServicesProductWebsocketsError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_services_product_websockets(configuration: &mut configuration::Configuration) -> Result<crate::models::WebsocketsResponseBodyGetAllServices, Error<GetServicesProductWebsocketsError>> {
    let local_var_configuration = configuration;
    ...
    let local_var_uri_str = format!("{}/enabled-products/v1/websockets/services", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetServicesProductWebsocketsError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
